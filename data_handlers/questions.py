...
from data_handlers.base import DataHandler
...
class QuestionsDataHandler(DataHandler):
    ...
    def create_table(self):
        """
        Creates the 'questions' table if it doesn't exist. Also resets the status of any questions marked as 'Обрабатывается' (Processing) to 'Не обработан' (Not processed).
        """
    ...
    def check_question_reservation(self, question_id: int, moderator_username: str) -> objects_types.QuestionDict:
        ...
        Returns:
            objects_types.QuestionDict: A dictionary representing the question data if the question is reserved by the specified moderator and its status is "Обрабатывается" (Processing). Returns an empty dictionary if no such reservation exists.
        """
        connection, cursor = self.get_attributes()
    ...
    def free_question_from_moderator(self, question_id: int):
        """
        Releases a question from a moderator's reservation. Sets the `answered_date` to NULL, the `status` to "Не обработан" (Not Processed), and clears the `moderator_username`.
    ...
    def get_first_unanswered_question(self, declined_questions: list[int]) -> objects_types.QuestionDict:
        ...
        Returns:
            objects_types.QuestionDict: A dictionary representing the first unanswered question found. Returns an empty dictionary if no unanswered questions are found. A question is considered unanswered if its status is "Не обработан" (Not Processed) or if it's marked as "Обрабатывается" (Processing) but the `answered_date` is older than `self.time_for_answer` seconds.
        """
    ...
    def get_users_statistics(self) -> dict[str, int]:
        """
        Retrieves statistics about moderators and the number of questions they have answered.

        Returns:
            dict[str, int]: A dictionary where keys are moderator usernames and values are the number of questions they've answered (status "Обработан"). Returns statistics only for the *first* moderator found based on the sorting criteria (count_questions DESC, then ASC). This is likely a bug – it should probably return statistics for *all* moderators.
        """
        connection, cursor = self.get_attributes()
        ...
        users_statistics = functions.get_db_line_dict([header[0] for header in cursor.description], cursor.fetchone())
    ...
    def mark_question_as_answered(self, question_id: int):
        """
        Marks a question as answered by updating its status to "Обработан" (Processed) and setting the `answered_date` to the current timestamp.
    ...
    def reserve_question_for_moderator(self, moderator_username: str, question_id: int):
        """
        Reserves a question for a specific moderator. Sets the `answered_date` to the current timestamp (which seems unusual for a reservation), the `status` to "Обрабатывается" (Processing), and the `moderator_username`.
